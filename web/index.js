...
export async function createServer(
  ...
  //* Handle API PAGE
  app.post("/api/pages", express.json(), verifyRequest(app), async (req, res) => {
    try {
      console.log("Body _post", req.body)
  ...
  app.get('/api/pages', async (req, res) => {
    try {
      ...
      const allPage = await Page.all({
        session: test_session
      });
      res.status(201).json(allPage)
    } catch (error) {
      console.log("Message:", error)
    }
  })
  ...
  app.delete('/api/pages/:id', async (req, res) => {
    try {
      console.log("ac...")
      const test_session = await Shopify.Utils.loadCurrentSession(req, res);
      const result = await Page.delete({
        session: test_session,
        id: 81015799943,
      })
  ...
  app.delete('/api/pages', express.json(), async (req, res) => {
    try {
      ...
      const test_session = await Shopify.Utils.loadCurrentSession(req, res);
      const deteleData = req.body.map(id => Page.delete({ session: test_session, id: parseInt(id) }))
      await Promise.all(deteleData)
      res.status(201).json({ success: 'true' })
    } catch (error) {
      console.log("Message: ", error)
    }
